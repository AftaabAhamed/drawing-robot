...
class HBController(Node):
    def __init__(self):
        ...
        self.subscription=self.create_subscription(Pose2D,f'/pen{2}_pose',self.pose_cb,10)
        # self.subscription_kpx=self.create_subscription(Float32,f'/kpx_{self.bot_no}',self.kpx_cb,10)
        # self.subscription_kpy=self.create_subscription(Float32,f'/kpy_{self.bot_no}',self.pose_cb,10)
        # self.subscription_kptheta=self.create_subscription(Float32,f'/kptheta_{self.bot_no}',self.pose_cb,10)

        self.publisher = self.create_publisher(Vector3, f'/cmd_vel/bot{self.bot_no}', 10)
        
        self.pose_publisher = self.create_publisher(Pose2D, f'/logging/bot{self.bot_no}/pose', 10)

        self.setx_publisher = self.create_publisher(Float32, f'/logging/bot{self.bot_no}/setx', 10)
        self.sety_publisher = self.create_publisher(Float32, f'/logging/bot{self.bot_no}/sety', 10)
    ...
    def init_params(self):
        self.declare_parameter( name=f'kpx',value=(rclpy.Parameter.Type.DOUBLE))
        self.declare_parameter( name=f'kpy',value=(rclpy.Parameter.Type.DOUBLE))
        self.declare_parameter( name=f'kptheta',value=(rclpy.Parameter.Type.DOUBLE))
        param=self.get_parameter(f'kpx')
        self.kpx=param.value
        param=self.get_parameter(f'kpy')
        self.kpy=param.value
        param=self.get_parameter(f'kptheta')
        self.kp_theta=param.value
    ...
    def on_params_changed(self,params):
        ...
        for param in params:
            if param.name==f'kpx':
                self.kpx=param.value
            elif param.name==f'kpy':
                self.kpy=param.value
            elif param.name==f'kpt':
                self.kp_theta=param.value
            else:
                continue
...
def main(args=None):
    ...
    while rclpy.ok():            
            ...
            goal_pubx = Float32()
            goal_puby = Float32()                
            goal_pubx.data = hb_controller.goal_x[hb_controller.goal_index]
            goal_puby.data = hb_controller.goal_y[hb_controller.goal_index]
            hb_controller.setx_publisher.publish(goal_pubx)
            hb_controller.sety_publisher.publish(goal_puby)
