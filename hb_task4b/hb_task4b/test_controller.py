...
class HBController(Node):
    def __init__(self):
        ...
        self.pid_y = PID(0, 0, 0, setpoint = 0)
        ...
        self.state_subscription=self.create_subscription(Bool,f'/state_{self.bot_no}',self.state_cb,10)
    ...
    def init_params(self):
        self.declare_parameter( name=f'kp',value=(rclpy.Parameter.Type.DOUBLE))
        self.declare_parameter( name=f'kd',value=(rclpy.Parameter.Type.DOUBLE))
        self.declare_parameter( name=f'ki',value=(rclpy.Parameter.Type.DOUBLE))
        param=self.get_parameter(f'kp')
        self.pid.Kp =param.value
        param=self.get_parameter(f'kd')
        self.pid.Kd=param.value
        param=self.get_parameter(f'ki')
    ...
    def on_params_changed(self,params):
        ...
        for param in params:
            if param.name==f'kp' :
                self.pid.Kp=param.value
            elif param.name==f'kd' :
                self.pid.Kd=param.value
            elif param.name==f'ki' :
                self.pid.Ki=param.value
            else:
                continue
    ...
    def state_cb(self, msg: Bool):
        ...
        while not self.bot_state:
            self.wheel.x = 90.0
            self.wheel.y = 90.0
            self.wheel.z = 90.0
            self.publisher.publish(self.wheel)
...
def main(args=None):
    ...
    while rclpy.ok():            
            ...
            hb_controller.pid.setpoint = 0

            cmd_vel_x = hb_controller.pid(error_x)
            cmd_vel_y = hb_controller.pid(error_y)
